...
def anchor_to(runners: Sequence[Runner], target_prob: Sequence[float],
    ...
    시뮬레이션은 예측을 **대체**하지 않고 **연출**한다. 화면에서 ◎ 본명마가
...
def fit_noise(runners: Sequence[Runner], distance: float,
    ...
    짧은 시뮬레이션을 반복하며 본명마의 시뮬 승률이 예측 승률과 맞는
...
def simulate(
    ...
    중앙값 한 판을 뽑으면, 본명마 승률이 30%인 경주에서는 열에 일곱은 다른 말이
    이기는 그림이 나온다. 같은 페이지에서 ◎ 를 달아 놓고 다른 말이 들어오는
    영상을 트는 셈이다. 그래서 **본명마가 이긴 판들 중 가장 전형적인 한 판**을
...
CONFIDENCE_LABELS = [
    (51, "강승부", "본명마의 우위가 뚜렷하고 출주마 이력도 충분합니다"),
