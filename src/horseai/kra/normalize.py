...
# 착순 칸에 실려 오는 상태 코드. 착순이 아니라 '그 경주에서 무슨 일이 있었나'다.
# 이걸 숫자 그대로 두면 99착으로 화면에 찍히고, 순위 지표에서는 꼴찌로 학습된다.
ORD_STATUS = {
    91: "실격",
    92: "주행중지",
    93: "출발제외",
...
# 이 중 '달리기는 했으나 성적이 남지 않은' 경우. 패배로 세는 것이 맞다.
ORD_RAN = {91, 92}
MAX_ORD = 30          # 실제 착순의 상한 (최대 출주 두수보다 넉넉히)
...
# 같은 경마장을 API 마다 다르게 부른다. 표기가 갈리면 조인이 조용히 실패하고,
# 데이터가 없는 게 아니라 '못 찾는' 상태가 된다 (실제로 부경/부산경남 때문에
# 경주 30%가 매칭에 실패한 적이 있다). 새 API 를 붙일 때마다 여기부터 확인한다.
MEET_CANON = {
    "부경": "부산경남", "부산": "부산경남", "부산경남": "부산경남",
    "영남": "부산경남",          # 일별훈련(API18_1) 표기
    "서울": "서울", "제주": "제주", "영천": "영천",
}
