...
한국마사회 공공데이터를 학습한 머신러닝 모델이 경주별 승률을 예측하고, Claude가
예상 코멘트를 작성하며, 정적 사이트로 자동 배포되는 파이프라인입니다.
...
2. **배당률은 피처가 아니라 채점 기준이다.** 배당률을 넣으면 적중률은 쉽게 오르지만
   모델이 인기 순위를 베끼게 됩니다. 대신 1인기(최저 단승배당)를 베이스라인으로 두고
   **시장을 이기는지**를 매 학습마다 측정합니다.
